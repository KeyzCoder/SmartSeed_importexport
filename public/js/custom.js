...
window.deleteCrop = function (farmerId) {
  ...
  if (confirm("Are you sure you want to delete this crop record?")) {
    fetch(`/api/crops/${farmerId}`, {
      // Changed from /api/farmers/${farmerId}/crops
      method: "DELETE",
      headers: {
    ...
    togglePassword.addEventListener("click", function () {
      if (passwordInput.type === "password") {
        ...
        togglePassword.innerHTML =
          '<i class="fas fa-eye-slash" style="font-size: 0.8em;"></i>'; // Change icon
      } else {
        passwordInput.type = "password";
        togglePassword.innerHTML =
          '<i class="fas fa-eye" style="font-size: 0.8em;"></i>'; // Change icon
      }
    });
  }
      ...
      if (farmersTable) {
        ...
        $("#saveFarmerBtn").click(function () {
          ...
          const formData = {
            ...
            extension_name: $("#extensionName").val().trim() || null,
            address: $("#address").val().trim(),
            contact_number: $("#contactNumber").val().trim(),
            crop_type: $("#cropType").val(),
            farm_size: parseFloat($("#farmSize").val()),
          };
          ...
          fetch("/api/farmers", {
            ...
            .finally(() => {
              // Reset button state
              saveBtn.prop("disabled", false).html("Save Farmer");
            });
        });
        ...
        $("#dataTable").on("click", ".view-btn", function () {
          ...
          Promise.all([
            ...
            .then(([farmer, crops]) => {
              ...
              $("#viewFarmerModal").find(".modal-body").html(farmerDetails);
        ...
        $("#updateFarmerBtn").click(function () {
          try {
            ...
            const formData = {
              ...
              farm_size: $("#editFarmSize").val()
                ? parseFloat($("#editFarmSize").val())
                : 0,
            };
...
$(document).ready(function () {
  ...
  $("#printDataBtn").click(function () {
    ...
    printButton.style.cssText =
      "position: fixed; top: 10px; right: 10px; padding: 10px; background: #4e73df; color: white; border: none; border-radius: 4px; cursor: pointer;";
...
function refreshFarmerView(farmerId) {
  ...
  Promise.all([
    ...
    .then(([farmer, crops]) => {
      const farmerDetails = `
            <div class="farmer-details mb-4 p-3 border rounded shadow-sm bg-light">
                <p><strong>Name:</strong> ${farmer.first_name} ${
        farmer.middle_name || ""
      } ${farmer.last_name} ${farmer.extension_name || ""}</p>
...
$(document).ready(function () {
  ...
  const yieldCtx = document
    .getElementById("yieldHistoryChart")
    .getContext("2d");
  ...
  function updateYieldHistory() {
    fetch("/api/yield-history")
      ...
      .then((data) => {
        const colors = {
          Cagayan: {
            irrigated: "rgba(78, 115, 223, 0.8)",
            rainfed: "rgba(28, 200, 138, 0.8)",
          },
          Isabela: {
            irrigated: "rgba(54, 185, 204, 0.8)",
            rainfed: "rgba(246, 194, 62, 0.8)",
          },
          "Nueva Vizcaya": {
            irrigated: "rgba(231, 74, 59, 0.8)",
            rainfed: "rgba(133, 135, 150, 0.8)",
          },
          Quirino: {
            irrigated: "rgba(246, 194, 62, 0.8)",
            rainfed: "rgba(231, 74, 59, 0.8)",
          },
          Batanes: {
            irrigated: "rgba(28, 200, 138, 0.8)",
            rainfed: "rgba(78, 115, 223, 0.8)",
          },
        };
...
$(document).ready(function () {
  ...
  const totalCtx = document
    .getElementById("totalProductionChart")
    .getContext("2d");
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: [], // Will be populated with moving average values
        },
      ],
    },
    options: {
  ...
  function calculateMovingAverage(data, windowSize) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (i < windowSize - 1) {
        // Not enough data points for the window
      ...
      .then((data) => {
        ...
        const totalProduction = data.years.map((year, index) => {
          ...
          Object.values(data.provinces).forEach((province) => {
            total +=
              (province.irrigated[index] || 0) + (province.rainfed[index] || 0);
          });
          return total;
        });

        // Calculate moving average (e.g., 3-year window)
        const movingAverage = calculateMovingAverage(totalProduction, 3);
        ...
        totalProductionChart.data.datasets[1].data = movingAverage;
